...
def oper(que, m_cpu, numb, que_b=False):
    if finish:
        ...
        print('\n' + str(numb) + '\nУмножение:' + print_tab(matrix1) + ' на ' + print_tab(matrix2) + ' Ответ: ' + str(
            numb) + print_tab(matric_otv) + '\n')
...
while True:
    ...
    kom = input(
        'Введите genr {number of mult. -1 for  inf} {sixe} {max} {min} для запуска в режиме автоматической генерации,\n'
        'file {first_file_name} {second_file_name} для запуска в режиме считывания из csv файлов (имена файлов '
        'вводить без формата),\nexit для выхода,\nstop (для остановки вывода): ').split(
        " ")
    if kom[0] == 'exit':
        try:
            if pros:
        ...
        lt = [load_table(kom[1]), load_table(kom[2])]
        if lt[0] == [] or lt[1] == []:
